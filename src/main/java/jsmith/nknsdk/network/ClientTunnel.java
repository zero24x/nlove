...
public class ClientTunnel {
	...
	public void startClient() throws NKNClientException {
		if (running)
			throw new IllegalStateException("Client has already started, cannot start again");
		running = true;
	...
	private Consumer<Integer> onCurrentHeightChanged;

	public void onCurrentHeightChanged(Consumer<Integer> consumer) {
		this.setOnCurrentHeightChanged(consumer);
	}
	...
	private boolean setupWsConnection() {
		...
		ws.setJsonMessageListener(json -> {
			if (json.has("Error") && json.getInt("Error") == ErrorCodes.WRONG_NODE) {
				...
				case "sendRawBlock": {
					...
					if (json.getInt("Error") == ErrorCodes.SUCCESS) {
						try {
							...
							synchronized (heightLock) {
								height = newHeight;
								this.onCurrentHeightChanged.accept(newHeight);
							}
						} catch (Exception e) {
							throw e;
							// LOG.warn("Failed to decode sendRawBlock: {} {}", json.toString(), e);
						}
				...
				default:
					LOG.debug("Got unknown message (action='{}'), ignoring", json.getString("Action"));
				}
			}
		});
	...
	public Consumer<Integer> getOnCurrentHeightChanged() {
		return onCurrentHeightChanged;
	}

	public void setOnCurrentHeightChanged(Consumer<Integer> onCurrentHeightChanged) {
		this.onCurrentHeightChanged = onCurrentHeightChanged;
	}
