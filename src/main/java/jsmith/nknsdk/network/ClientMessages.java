...
public class ClientMessages extends Thread {
	...
	public void close() {
		if (!running)
			throw new IllegalStateException("Client is not (yet) running, cannot close");
			...
			for (MessageJob j : waitingForReply) {
				if (j.messageID.equals(replyTo)) {
					...
					if (type == MessagesP.PayloadType.TEXT) {
						try {
							j.ack.set(indexOf, new NKNClient.ReceivedMessage(from, messageID, encrypted, MessagesP.PayloadType.TEXT, MessagesP.TextData.parseFrom(data).getText()));
						} catch (InvalidProtocolBufferException e) {
							LOG.warn("Received packet is of type TEXT but does not contain valid text data");
						}
					} else if (type == MessagesP.PayloadType.BINARY) {
						j.ack.set(indexOf, new NKNClient.ReceivedMessage(from, messageID, encrypted, MessagesP.PayloadType.BINARY, data));
					} else if (type == MessagesP.PayloadType.ACK) {
						j.ack.set(indexOf, new NKNClient.ReceivedMessage(from, messageID, false, MessagesP.PayloadType.ACK, null));
					}
	...
	public List<CompletableFuture<NKNClient.ReceivedMessage>> sendMessageAsync(List<String> destination, ByteString replyTo, Object message)
			throws NKNClientException.UnknownObjectType {
	...
	public List<CompletableFuture<NKNClient.ReceivedMessage>> sendMessageAsync(List<String> destination, ByteString replyTo, MessagesP.PayloadType type, ByteString message) {
		...
		final MessagesP.Payload payload = MessagesP.Payload.newBuilder().setType(type).setPid(messageID).setReplyToPid(replyToMessageID)
				.setData(ClientEnc.encryptMessage(destination, message, ct.identity.wallet, encryptionLevel)).setNoAck(noAck).build();
	...
	private List<CompletableFuture<NKNClient.ReceivedMessage>> sendOutboundMessage(List<String> destination, ByteString messageID, ByteString payload) {
		if (destination.size() == 0)
			throw new IllegalArgumentException("At least one address is required for multicast");

		final ArrayList<CompletableFuture<NKNClient.ReceivedMessage>> promises = new ArrayList<>();
		for (String identity : destination) {
			if (identity == null || identity.isEmpty())
				throw new IllegalArgumentException("Destination identity is null or empty");
			promises.add(new CompletableFuture<>());
		}

		final MessagesP.ClientMsg.Builder clientToNodeMsg = MessagesP.ClientMsg.newBuilder().setPayload(payload).addAllDests(destination).setMaxHoldingSeconds(0);

		ClientEnc.signOutboundMessage(clientToNodeMsg, ct);

		final MessagesP.Message msg = MessagesP.Message.newBuilder().setMessage(clientToNodeMsg.build().toByteString()).setMessageType(MessagesP.MessageType.CLIENT_MSG).build();

		if (!running)
			throw new IllegalStateException("Client is not running, cannot send messages.");
	...
	public void sendAckMessage(String destination, ByteString replyTo) {
		final MessagesP.Payload payload = MessagesP.Payload.newBuilder().setType(MessagesP.PayloadType.ACK).setPid(ByteString.copyFrom(Crypto.nextRandom4B()))
				.setReplyToPid(replyTo).setNoAck(true).build();

		final MessagesP.ClientMsg binMsg = MessagesP.ClientMsg.newBuilder().setDest(destination).setPayload(payload.toByteString()).setMaxHoldingSeconds(0).build();
